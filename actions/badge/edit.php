...
if ($badge->created || isset($_POST['publish']))
    $created = true;
else
    $created = false;
...
if ($badge->getSubtype() == "badge" && $badge->canEdit()) {
    ...
    $previous_file = elgg_get_entities_from_relationship(array('relationship' => 'badge_file_link', 'relationship_guid' => $badgepost, 'inverse_relationship' => false, 'type' => 'object', 'limit' => 0));

    if (!empty($previous_file)) {
        $previous_file_counter = count($previous_file);
    } else {
        $previous_file_counter = 0;
    }

// Cache to the session
    elgg_make_sticky_form('edit_badge');
...
// Make sure the title is not blank
    ...
    if (strcmp($badge_type, 'badge_automatic') == 0) {
        ...
        foreach ($contest_position_text as $one_position_text) {
            if (strlen($one_position_text) > 10) {
                register_error(elgg_echo("badge:contests_text_too_long"));
                forward($_SERVER['HTTP_REFERER']);
            }
        }
    }
    ...
    if (strcmp($badge_type, "badge_contest") == 0) {
        ...
        if (count($files) != $winners && !$new_image) {
    ...
    // Before we can set metadata, we need to save the badge post
    if (!$badge->save()) {
    ...
    // Previous files
    if ($previous_file_counter > 0 && $new_image) {
        foreach ($previous_file as $one_previous_file) {
            $deleted = $one_previous_file->delete();
            if (!$deleted) {
                register_error(elgg_echo('badge:filenotdeleted'));
                forward($_SERVER['HTTP_REFERER']);
            }
    ...
    //Update gamepoints associated to the badge
    ...
    if ($badge->badge_manual_gamepoints) {
        ...
        foreach ($badge_users as $badge_user) {
            ...
            if (!$gamepoints) {
                gamepoints_add($user_guid, $badge_num_manual_gamepoints, $badge->guid . "-" . $user_guid, $badge->group_guid, false, $title);
            } else {
                gamepoints_remove($gamepoints->guid);
                gamepoints_add($user_guid, $badge_num_manual_gamepoints, $badge->guid . "-" . $user_guid, $badge->group_guid, false, $title);
            }
        }
    ...
    //Delete gamepoints when changing to a no-gamepoints-badge
    if (!$badge->badge_manual_gamepoints) {
        foreach ($badge_users as $badge_user) {
            $user_guid = $badge_user->getGUID();
            $access = elgg_set_ignore_access(true);
    ...
    // System message
    system_message(elgg_echo("badge:updated"));

    //River
    //add_to_river('river/object/badge/update','update',$user_guid,$badgepost);

<div class="contentWrapper">
    ...
    if (!elgg_is_sticky_form('edit_badge')) {
        $title = $badge->title;
        $desc = $badge->desc;
        $surprise = $badge->surprise;
        $global_visibility = $badge->global_visibility;
        $badge_type = $badge->badge_type;
        $badge_images = $badge->badge_images;
        $badge_visibility = $badge->badge_visibility;
        $text_badge_selector = $badge->text_badge_selector;
        $image_alternative_text = $badge->image_text;
        if (strcmp($badge_type, 'badge_automatic') == 0) {
            $badge_merits = $badge->badge_merits;
            $badge_gamepoints = $badge->badge_gamepoints;
            $badge_activitypoints = $badge->badge_activitypoints;
            if ($badge_gamepoints)
                $badge_num_gamepoints = $badge->badge_num_gamepoints;
            if ($badge_activitypoints)
                $badge_num_activitypoints = $badge->badge_num_activitypoints;
            if ($badge_merits) {
                $assignment_activated = $badge->assignment_activated;
                $assignment_test_activated = $badge->assignment_test_activated;
                if ($assignment_activated) {
                    ...
                    if (strcmp($assignment, 'badge_assignment_threshold') == 0) {
                        ...
                        if (is_array($type_grading)) {
                            if (in_array('badge_type_grading_marks', $type_grading))
                                $marks_average_threshold = $badge->marks_average_threshold;
                            if (in_array('badge_type_grading_game_points', $type_grading))
                                $total_game_points_threshold = $badge->total_game_points_threshold;
                        } else {
                            if (strcmp('badge_type_grading_marks', $type_grading) == 0)
                                $marks_average_threshold = $badge->marks_average_threshold;
                            if (strcmp('badge_type_grading_game_points', $type_grading) == 0)
                                $total_game_points_threshold = $badge->total_game_points_threshold;
                        }
                    }
            ...
            $badge_manual_gamepoints = $badge->badge_manual_gamepoints;
            if ($badge_manual_gamepoints) {
                $badge_num_manual_gamepoints = $badge->badge_num_manual_gamepoints;
            }
        } else if (strcmp($badge_type, 'badge_contest') == 0) {
            $selected_contests_guids = $badge->selected_contests_guids;
            $contest_position_text = $badge->contest_position_text;
            $contest_position_color = $badge->contest_position_color;
        }

        $tags = $badge->tags;
        $access_id = $badge->access_id;
    } else {
        $title = elgg_get_sticky_value('edit_badge', 'title');
        $desc = elgg_get_sticky_value('edit_badge', 'desc');
        $surprise = elgg_get_sticky_value('edit_badge', 'surprise');
        $global_visibility = elgg_get_sticky_value('edit_badge', 'global_visibility');
        $badge_type = elgg_get_sticky_value('edit_badge', 'badge_type');
        $badge_images = elgg_get_sticky_value('edit_badge', 'badge_images');
        $text_badge_selector = elgg_get_sticky_value('edit_badge', 'text_badge_selector');
        $image_alternative_text = elgg_get_sticky_value('edit_badge', 'image_alternative_text');
        $badge_visibility = elgg_get_sticky_value('edit_badge', 'badge_visibility');
        if (strcmp($badge_type, 'badge_automatic') == 0) {
            $badge_merits = elgg_get_sticky_value('edit_badge', 'badge_merits');
            $badge_gamepoints = elgg_get_sticky_value('edit_badge', 'badge_gamepoints');
            $badge_activitypoints = elgg_get_sticky_value('edit_badge', 'badge_activitypoints');
            if ($badge_gamepoints)
                $badge_num_gamepoints = elgg_get_sticky_value('edit_badge', 'badge_num_gamepoints');
            if ($badge_activitypoints)
                $badge_num_activitypoints = elgg_get_sticky_value('edit_badge', 'badge_num_activitypoints');
            if ($badge_merits) {
                $assignment_activated = elgg_get_sticky_value('edit_badge', 'assignment_activated');
                $assignment_test_activated = elgg_get_sticky_value('edit_badge', 'assignment_test_activated');
                if ($assignment_activated) {
                    $assignment = elgg_get_sticky_value('edit_badge', 'assignment');
                    if (strcmp($assignment, 'badge_assignment_threshold') == 0) {
                        $type_grading = elgg_get_sticky_value('edit_badge', 'type_grading');
                        if (sizeOf($type_grading) > 1) {
                            $marks_average_threshold = elgg_get_sticky_value('edit_badge', 'marks_average_threshold');
                            $total_game_points_threshold = elgg_get_sticky_value('edit_badge', 'total_game_points_threshold');
                            $selected_tasks_guids_array = elgg_get_sticky_value('edit_badge', 'tasks_both_marks_game_points_assignment_1_guids');
                        } else {
                            if (in_array('badge_type_grading_marks', $type_grading)) {
                                $marks_average_threshold = elgg_get_sticky_value('edit_badge', 'marks_average_threshold');
                                $selected_tasks_guids_array = elgg_get_sticky_value('edit_badge', 'tasks_marks_assignment_1_guids');
                            }
                            if (in_array('badge_type_grading_game_points', $type_grading)) {
                                $total_game_points_threshold = elgg_get_sticky_value('edit_badge', 'total_game_points_threshold');
                                $selected_tasks_guids_array = elgg_get_sticky_value('edit_badge', 'tasks_game_points_assignment_1_guids');
                            }
                        }
                    } else if (strcmp($assignment, 'badge_assignment_not_threshold') == 0) {
                        $selected_tasks_guids_array = elgg_get_sticky_value('edit_badge', 'tasks_marks_assignment_0_guids');
                    } else {
                        $selected_tasks_guids_array = elgg_get_sticky_value('edit_badge', 'tasks_answering_assignment_2_guids');
                    }
                    $selected_tasks_guids = implode(',', $selected_tasks_guids_array);
                }
                if ($assignment_test_activated) {
                    $assignment_test = elgg_get_sticky_value('edit_badge', 'assignment_test');
                    if (strcmp($assignment_test, 'badge_assignment_threshold_test') == 0) {
                        $type_grading_test = elgg_get_sticky_value('edit_badge', 'type_grading_test');
                        if (sizeOf($type_grading_test) > 1) {
                            $marks_average_threshold_test = elgg_get_sticky_value('edit_badge', 'marks_average_threshold_test');
                            $total_game_points_threshold_test = elgg_get_sticky_value('edit_badge', 'total_game_points_threshold_test');
                            $selected_tests_guids_array = elgg_get_sticky_value('edit_badge', 'tests_both_marks_game_points_assignment_1_guids');
                        } else {
                            if (in_array('badge_type_grading_marks_test', $type_grading_test)) {
                                $marks_average_threshold_test = elgg_get_sticky_value('edit_badge', 'marks_average_threshold_test');
                                $selected_tests_guids_array = elgg_get_sticky_value('edit_badge', 'tests_marks_assignment_1_guids');
                            }
                            if (in_array('badge_type_grading_game_points_test', $type_grading_test)) {
                                $total_game_points_threshold_test = elgg_get_sticky_value('edit_badge', 'total_game_points_threshold_test');
                                $selected_tests_guids_array = elgg_get_sticky_value('edit_badge', 'tests_game_points_assignment_1_guids');
                            }
                        }
                    } else if (strcmp($assignment_test, 'badge_assignment_not_threshold_test') == 0) {
                        $selected_tests_guids_array = elgg_get_sticky_value('edit_badge', 'tests_marks_assignment_0_guids');
                    } else {
                        $selected_tests_guids_array = elgg_get_sticky_value('edit_badge', 'tests_answering_assignment_2_guids');
                    }
                    $selected_tests_guids = implode(',', $selected_tests_guids_array);
                }
            }
            $selected_badges_guids_array = elgg_get_sticky_value('edit_badge', 'badges_guids');
            $selected_badges_guids = implode(',', $selected_badges_guids_array);
        } else if (strcmp($badge_type, 'badge_manual') == 0) {
            $badge_manual_gamepoints = elgg_get_sticky_value('edit_badge', 'badge_manual_gamepoints');
            if ($badge_manual_gamepoints) {
                $badge_num_manual_gamepoints = elgg_get_sticky_value('edit_badge', 'badge_num_manual_gamepoints');
            }
        } else if (strcmp($badge_type, 'badge_contest') == 0) {
            $selected_contests_guids_array = elgg_get_sticky_value('edit_badge', 'contests_guids');
            ...
            for ($i = 0; $i < $winners; $i++) {
                $contest_position_text[$i] = elgg_get_sticky_value('edit_badge', 'contest_position_' . ($i + 1) . '_text');
                $contest_position_color[$i] = elgg_get_sticky_value('edit_badge', 'contest_position_' . ($i + 1) . '_color');
            }
        }
        $tags = elgg_get_sticky_value('edit_badge', 'badgetags');
        $access_id = elgg_get_sticky_value('edit_badge', 'access_id');
    }

    elgg_clear_sticky_form('edit_badge');

    if (strcmp($marks_average_threshold, "") == 0)
        $marks_average_threshold = 80;

    if (strcmp($total_game_points_threshold, "") == 0)
        $total_game_points_threshold = 100;

    if (strcmp($marks_average_threshold_test, "") == 0)
        $marks_average_threshold_test = 80;

    if (strcmp($total_game_points_threshold_test, "") == 0)
        $total_game_points_threshold_test = 100;

    if (strcmp($badge_num_gamepoints, "") == 0)
        $badge_num_gamepoints = 100;

    if (strcmp($badge_num_manual_gamepoints, "") == 0)
        $badge_num_manual_gamepoints = 100;

    if (strcmp($badge_num_activitypoints, "") == 0)
    ...
    foreach ($contests as $one_contest) {
        ...
        foreach ($badges as $one_badge) {
            if (strcmp($one_badge->badge_type, "badge_contest") == 0)
                if ($one_badge->getGUID() != $badge->getGUID() && $one_badge->selected_contests_guids == $one_contest_guid)
    ...
    if ($badge_merits && ($container->tasks_enable == 'yes' || $container->tests_enable == 'yes' || $container->activitypoints_enable == 'yes' || $container->gamepoints_enable == 'yes')) {
    ...
    if (is_array($type_grading)) {
        if (in_array($op_type_grading[0], $type_grading) && in_array($op_type_grading[1], $type_grading))
            ...
            if (in_array($op_type_grading[1], $type_grading))
                $display_game_points = true;
        }
    } else {
        if (strcmp($op_type_grading[0], $type_grading) == 0)
            $display_marks = true;
        if (strcmp($op_type_grading[1], $type_grading) == 0)
            $display_game_points = true;
    }
    ...
    if ($display_both) {
        ...
        if ($display_marks && $container->socialwire_marks_enable == 'yes') {
            $checked_radio_type_grading_0 = "checked = \"checked\"";
            $style_display_type_grading_0 = "display:block";
            $style_display_marks_average_threshold = "display:block";
        }
        if ($display_game_points && $container->gamepoints_enable == 'yes') {
            $checked_radio_type_grading_1 = "checked = \"checked\"";
    ...
    $display_marks_test = false;
    $display_game_points_test = false;
    $display_both_test = false;

    if (is_array($type_grading_test)) {
        if (in_array($op_type_grading_test[0], $type_grading_test) && in_array($op_type_grading_test[1], $type_grading_test))
            $display_both_test = true;
        else {
            if (in_array($op_type_grading_test[0], $type_grading_test))
                $display_marks_test = true;
            if (in_array($op_type_grading_test[1], $type_grading_test))
                $display_game_points_test = true;
        }
    } else {
        if (strcmp($op_type_grading_test[0], $type_grading_test) == 0)
            $display_marks_test = true;
        if (strcmp($op_type_grading_test[1], $type_grading_test) == 0)
            $display_game_points_test = true;
    }

    if ($display_both_test) {
        ...
        if ($display_marks_test && $container->socialwire_marks_enable == 'yes') {
            $checked_radio_type_grading_test_0 = "checked = \"checked\"";
            $style_display_type_grading_test_0 = "display:block";
            $style_display_marks_average_threshold_test = "display:block";
        }
        if ($display_game_points_test && $container->gamepoints_enable == 'yes') {
            $checked_radio_type_grading_test_1 = "checked = \"checked\"";
    ...
    <form action="<?php echo elgg_get_site_url() . "action/" . $action ?>" name="edit_badge"
        ...
        <p>
            <b><?php echo elgg_echo("badge:icon_label"); ?></b><br>
        ...
        <p>
            <b><?php echo elgg_echo("badge:badge_type_label"); ?>
            </b>
            <br>
...
<input type="hidden" name="badgepost" value="<?php echo $badgepost; ?>">
...
<script language="javascript">
    ...
    function check_badge_type(badge_type) {
        var radiobuttons = document.getElementsByName('badge_type');
        if (badge_type == 'badge_contest') {
    ...
    <?php echo "check_badge_type(\"$badge_type\");";?>
